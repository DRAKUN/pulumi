...
// GetUpdatedStackTags returns the stack's existing tags merged with fresh tags from the environment
// and Pulumi.yaml file.
func GetUpdatedStackTags(ctx context.Context, s Stack) (map[apitype.StackTagName]string, error) {
	...
	dynamicTags, err := GetCurrentStackEnvironmentTags()
	if err != nil {
		return nil, err
	}
...
// GetCurrentStackEnvironmentTags returns the set of tags for the "current" stack, based on the environment
// and Pulumi.yaml file.
func GetCurrentStackEnvironmentTags() (map[apitype.StackTagName]string, error) {
...
func ValidateStackProperties(stack string, tags map[apitype.StackTagName]string) error {
	...
	if err := ValidateStackTags(tags); err != nil {
		return err
	}
